...
async function run() {
  ...
  const sourceEmailsId = await createDb(PARENT!, 'Source Emails', {
    Title:               title('Email subject line — used as the record identifier.'),
    ...
    CC:                  text('CC recipients, if any.'),
    'BCC Indicator':     check('True when capture inbox was not in To or CC — indicates BCC delivery.'),
    'Received Date':     date('Date the email was received by the capture inbox.'),
    Subject:             text('Original email subject line (richText copy for querying).'),
    'Thread Reference':  text('In-Reply-To or first References header — links threaded emails.'),
    ...
    'Source Category':   sel(['Meeting Asset', 'Operational', 'Unknown'],
    ...
    'Raw Snippet':       text('First 500 characters of the email body for quick reference.'),
    'Detected Links':    text('Google Drive URLs extracted from this email, if any.'),
    'Error Log':         text('Error messages from failed processing attempts.'),
    'Processed At':      date('When this email was successfully processed.'),
  ...
  const meetingsId = await createDb(PARENT!, 'Meetings', {
    'Meeting Title':          title('Auto-generated from the email subject or Google Meet title.'),
    ...
    'Google Calendar Link':   url('Google Calendar event URL, if extracted from the notification email.'),
    'Google Meet Link':       url('Google Meet room URL for this meeting.'),
    'Recording Link':         url('Google Drive URL of the meeting recording, if received and accessible.'),
    'Transcript Link':        url('Google Drive URL of the meeting transcript, if received and accessible.'),
    'Notes Link':             url('Google Drive URL of the Gemini meeting notes, if received and accessible.'),
    'Recording Access Status':  sel(['Unknown', 'Confirmed', 'Needs Access', 'Denied'],
                                  'Whether Sera can access the recording file.'),
    'Transcript Access Status': sel(['Unknown', 'Confirmed', 'Needs Access', 'Denied'],
                                  'Whether Sera can access the transcript file.'),
    'Notes Access Status':      sel(['Unknown', 'Confirmed', 'Needs Access', 'Denied'],
                                  'Whether Sera can access the notes file.'),
    ...
    Summary:                  text('AI-generated summary of the meeting content.'),
    'Canon Review Required':  check('Set by Sera when extraction contains governance-significant decisions.'),
    'Sensitive Review Required': check('Set by Sera when extraction contains sensitive content.'),
    'Last Processed At':      date('When this meeting was last processed by the worker.'),
    'Automation Log':         text('Internal log of processing steps and errors.'),
  ...
  const meetingAssetsId = await createDb(PARENT!, 'Meeting Assets', {
    ...
    'Google Drive File ID': text('Google Drive file ID extracted from the asset URL.'),
    'Google Drive Link':  url('Google Drive URL for this asset.'),
    'Access Status':      sel(['Unknown', 'Confirmed', 'Needs Access', 'Denied'],
                            'Whether Sera can read this file from Google Drive.'),
    'Processing Status':  sel(['Pending', 'Processing', 'Processed', 'Needs Access', 'Failed', 'Manual Review'],
                            'Current processing state of this asset.'),
    'Received At':        date('Date the asset notification email was received.'),
    'Processed At':       date('When this asset was successfully downloaded and extracted.'),
    'Error Message':      text('Error messages from failed processing attempts.'),
    'Retry Count':        num('Number of times Sera has retried accessing this asset.'),
    'Next Retry At':      date('When Sera will next attempt to access this asset.'),
