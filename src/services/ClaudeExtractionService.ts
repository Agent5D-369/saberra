...
- "profile_updates": array of people or organizations for whom you have genuinely new information not likely already on file. Do not extract a profile merely because someone attended a meeting or was mentioned by name - only extract when you have new contact details, a new role, a new organizational relationship, or substantive new context that enriches their record. Each entry: { "name": string, "profile_type": "Person"|"Organization"|"Both", "email": string|null, "role_title": string|null, "primary_role": string|null (their specific role within this organization's governance structure, e.g. "Stewardship Circle facilitator" - distinct from their external role_title), "organization": string|null, "circle_affiliation": string[], "tags": string[], "location": string|null, "website": string|null, "linkedin": string|null, "relationship_to_org": "Member"|"Partner"|"Vendor"|"Advisor"|"Funder"|"Contact"|"Community"|"Alumni"|"Government"|"Unknown", "membership_type": "Founding Member"|"Full Member"|"Associate Member"|"Guest"|"Steward"|"Partner"|null (membership tier - extract only when explicitly stated), "primary_sector": "Sector 1 — Health & Holistic Wellness"|"Sector 2 — Governance & Justice"|"Sector 3 — Culture & Spirit"|"Sector 4 — Learning & Innovation"|"Sector 5 — Ecology & Infrastructure"|"Sector 6 — Economy & Exchange"|"Sector 7 — Media & Technology"|null, "context_notes": string|null, "referred_by": string|null, "connected_to": string[] (names of OTHER profiles this person is explicitly described as having a peer relationship with - co-founders, partners, collaborators; do NOT use for hierarchical org membership or referrals; only set when the source explicitly describes a direct relationship between two named individuals; empty array if none), "source_evidence": string }. Only include profiles where you have at least a name. Do not invent contact details. "tags" should reflect skills or domains evident from context (choose from: Leadership, Legal, Finance, Agriculture, Education, Communications, Operations, Governance, Technical, Community, Land Stewardship, Fundraising). "circle_affiliation" should list the names of the organization's governance circles this person is a member of or affiliated with, as mentioned in the source. "primary_sector" should reflect the person's primary domain of contribution - infer from their role, circle affiliation, or work described; use null when genuinely unclear.
...
- "risks": array of risks, concerns, and organizational health warning signals explicitly raised. SEVERITY FLOOR: only extract High or Medium severity risks — skip Low severity entirely. NEVER flag Sera's own processing failures as organizational risks: if an email was truncated, a document was inaccessible, a URL could not be resolved, or Sera could not fetch content — that is a system note for Processing Events, not an org risk. Each entry: { "risk": string (for Collapse Pattern risks, prefix the title with the pattern name in brackets, e.g. "[Burnout] Maria has reported feeling overwhelmed for three consecutive weeks"), "category": "Operational"|"Financial"|"Legal"|"Governance"|"Interpersonal"|"Technical"|"Collapse Pattern"|"Unknown", "collapse_pattern_type": "Interpersonal Conflict & Human Complexity"|"No Shared Vision, No Shared Future"|"Poor Governance & Power Shadows"|"Financial Fragility"|"Burnout & Loss of Commitment"|"The Wrong People Problem"|"The Scale Trap"|null (required when category is "Collapse Pattern", null otherwise), "severity": "High"|"Medium"|"Low", "owner": string|null (person name only - use when a specific individual is named), "owner_role": string|null (role name responsible for tracking this risk, e.g. 'Risk Steward', 'Circle Lead'; prefer role over person for Teal governance), "suggested_mitigation": string|null, "circles": string[] (names of CCOS circles this risk affects - use circle names from circle_updates or mentioned in the source; empty array if none), "source_evidence": string }. For Collapse Pattern detection: scan the source for early warning signals of organizational collapse and create a separate risk entry for each distinct signal found. The seven collapse patterns in canonical order (use this numbering for reference): 1-Interpersonal Conflict & Human Complexity, 2-No Shared Vision No Shared Future, 3-Poor Governance & Power Shadows, 4-Financial Fragility, 5-Burnout & Loss of Commitment, 6-The Wrong People Problem, 7-The Scale Trap. Signal language by pattern - 1-Interpersonal Conflict & Human Complexity: named tension between individuals, "difficult dynamic", "I need to name something", withdrawal or non-participation signals, "there is tension between", unresolved interpersonal dynamics. 2-No Shared Vision No Shared Future: disagreement about purpose or direction, "I thought we agreed", "why are we doing this", conflicting interpretations of goals, "that's not what we signed up for". 3-Poor Governance & Power Shadows: "we already decided this", role ambiguity, "who is responsible for", repeated same-type unresolved decisions, governance process bypassed or ignored, "there's no clear process for". 4-Financial Fragility: cash flow concerns, runway language, "we can't afford", vendor payment pressure, urgent fundraising pressure, unreciprocated labor signals. 5-Burnout & Loss of Commitment: "overwhelmed", "too much on my plate", recurring missed commitments by the same person, "I can't keep up", extended absences, "I need to step back". 6-The Wrong People Problem: skill gap language, "we need someone who can", misalignment between role requirements and holder capabilities, "this isn't working", cultural-fit concerns. 7-The Scale Trap: "we can't keep up with the volume", "too many meetings", cross-circle coordination failures, "things are falling through the cracks", capacity bottlenecks preventing delivery. Only flag Collapse Pattern risks when the language is explicit and specific - do not flag normal governance tension, routine workload discussions, or ordinary challenges as collapse signals.
...
Return JSON with exactly these keys:
{
  ...
  async writeToNotion(
    ...
    const data = extraction as {
      ...
      profile_updates?: Array<{ name: string; profile_type?: string; email?: string; role_title?: string; primary_role?: string; organization?: string; circle_affiliation?: string[]; tags?: string[]; location?: string; website?: string; linkedin?: string; relationship_to_org?: string; membership_type?: string; engagement_status?: string; primary_sector?: string; context_notes?: string; referred_by?: string; connected_to?: string[]; source_evidence?: string }>;
    ...
    const primaryRoleQueue:       Array<{ profileId: string; roleName: string }> = [];
    ...
    const connectedToQueue:       Array<{ profileId: string; connectedNames: string[] }> = [];
    ...
    for (let p of (Array.isArray(data.profile_updates) ? data.profile_updates : [])) {
      ...
      try {
        ...
        if (p.primary_role) primaryRoleQueue.push({ profileId, roleName: p.primary_role });
        ...
        const connectedNames = Array.isArray(p.connected_to) ? p.connected_to.filter(Boolean).map(String) : [];
        if (connectedNames.length) connectedToQueue.push({ profileId, connectedNames });
      } catch (err) {
    ...
    // Flush Primary Role relations now that roleIdMap is populated
    for (const { profileId, roleName } of primaryRoleQueue) {
      try {
        const roleId = await resolve(roleIdMap, notion.dbIds.roles, 'Role Name', roleName);
        if (roleId) await notion.updatePage(profileId, { 'Primary Role': N.relation([roleId]) });
      } catch {}
    }

    // Flush Connected To relations — dual_property so Notion auto-populates the back-ref
    for (const { profileId, connectedNames } of connectedToQueue) {
      try {
        const ids = (await Promise.all(
          connectedNames.map(n => resolve(profileIdMap, notion.dbIds.profiles, 'Name', n))
        )).filter((id): id is string => Boolean(id));
        if (ids.length) await notion.updatePage(profileId, { 'Connected To': N.relation(ids) });
      } catch {}
    }
