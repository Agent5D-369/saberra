...
export const DATABASE_SCHEMAS: Record<string, { title: string; properties: PropertySchema }> = {
  ...
  profiles: {
    ...
    properties: {
      ...
      'Relationship to Org': select(['Member', 'Partner', 'Vendor', 'Advisor', 'Funder', 'Contact', 'Community', 'Alumni', 'Government', 'Unknown']),
  ...
  processingEvents: {
    ...
    properties: {
      Event: title(),
      'Tenant ID': richText(),
      'Event Type': select(['Poll Start', 'Poll Complete', 'Email Ingested', 'Access Check', 'Extraction Start', 'Extraction Complete', 'Extraction Failed', 'Retry Queued', 'Access Requested', 'Manual Review Flagged', 'Admin Action', 'Heartbeat', 'Scheduled Task']),
      Service: select(['Worker', 'API', 'Dashboard']),
      Status: select(['Success', 'Warning', 'Error', 'Info']),
      Timestamp: date(),
      'Source Email ID': richText(),
      'Meeting ID': richText(),
      'Token Count': number(),
      Details: richText(),
    },
